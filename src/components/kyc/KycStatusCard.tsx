...
import { 
  ...
  Shield, FileText, ExternalLink, Copy, Info, ArrowRight, Camera, Upload, Edit
} from 'lucide-react';
...
export function KycStatusCard({ kycSession, onRefresh, userId, onStartResubmission }: Props) {
  ...
  // Analyze rejection using helper
  const rejectionAnalysis = analyzeRejection(
          ...
          {kycSession.status === 'REJECTED' && (() => {
            ...
            return (
              <div className="space-y-6">
                ...
                {rejectionAnalysis.requirements.length > 0 && (
                  <div className="rounded-lg border bg-muted/30 p-4">
                ...
                {/* CTA: Actions based on rejection type */}
                {rejectionAnalysis.canResubmit && hasAttemptsLeft ? (
                  <div className="space-y-4">
                    ...
                    {rejectionAnalysis.needsDocumentUpload && (
                      <div>
                        <Button 
                          variant={rejectionAnalysis.needsSdk ? "outline" : "default"}
                          className="w-full"
                          size="lg"
                          onClick={() => router.push('/kyc/resubmit-documents')}
                        >
                    ...
                    {/* Form Edit for applicant data issues */}
                    {rejectionAnalysis.needsFormEdit && (
                      <div>
                        <Button 
                          variant="outline"
                          className="w-full"
                          ...
                          onClick={() => {
                            if (onStartResubmission) {
                              ...
                              toast.info('Form editing not available yet');
                            }
                          }}
                        >
