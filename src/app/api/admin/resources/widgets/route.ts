...
const schema = z.object({
  ...
  defaultFiat: z.string(),
  defaultCrypto: z.string().optional(),
  theme: z.record(z.any()),
  minKycForMethods: z.record(z.any()),
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const item = await prisma.widgetConfig.create({ data: validated });

    const adminId = await getCurrentUserId();
    if (adminId) await auditService.logAdminAction(adminId, AUDIT_ACTIONS.SETTINGS_UPDATED, 'WidgetConfig', item.code, {}, validated);
